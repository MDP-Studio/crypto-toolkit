...
## Phase 21: AES Interop and Lifecycle Trust Signal

Driver: P2 audit finding flagged "good educational disclosure, but trust signal would improve with explicit interoperability and crypto-lifecycle guardrails." NIST SP 800-38D is being revised and OWASP crypto guidance now emphasizes lifecycle controls, not only primitive correctness. Fix the gap without overstating production readiness.

### Cross-implementation parity
- [x] Shared vector pack at `src/data/aes-interop-vectors.json` (FIPS 197 Appendix B + C.1 for ECB; NIST SP 800-38D Test Cases 2, 3, 4 for GCM).
- [x] Vitest interop suite `src/__tests__/aes-interop.test.ts`: each vector through the toolkit AND WebCrypto, plus a toolkit-vs-WebCrypto agreement assertion.
- [x] Live matrix generator `scripts/generate-aes-interop-matrix.mjs` drives Node `webcrypto` over the same vectors and emits `docs/aes-interop-matrix.md`. Wired as `npm run interop` and into `npm run ci`.
- [x] Python reproducibility helper `scripts/aes-interop/verify_python.py` using `cryptography.hazmat` AES and AESGCM.
- [x] OpenSSL reproducibility helper `scripts/aes-interop/verify_openssl.sh` using `openssl enc -aes-128-ecb -nopad`; AES-GCM AEAD tag parity is explicitly deferred to the Python helper because OpenSSL `enc` does not expose reliable AEAD tag verification.

### Lifecycle guidance
- [x] `docs/aes-lifecycle.md` covers nonce uniqueness (SP 800-38D §8.3), AAD metadata binding, rotation triggers, envelope structure, KEK/DEK separation, algorithm agility, and an explicit "what this toolkit deliberately does NOT do" section. Tracks NIST SP 800-57 Part 1 Rev 5 and OWASP Cryptographic Storage / Key Management cheat sheets.

### Wire-up
- [x] `module-assurance.json` updated for `aes` and `aes-gcm`: new spec sections, vector sources, test IDs, and lifecycle limitation referencing the new doc.
- [x] `docs/assurance-matrix.md` regenerated by `npm run assurance`.
- [x] README surfaces both the interop matrix and the lifecycle doc, plus the new "AES interop" row in the test vectors table.

### Verification
- [x] `npm test` - 137 passed (was 122); new `aes-interop` suite contributes 15 tests over 5 vectors, prior suites unaffected
- [x] `npm run interop` - matrix regenerates with WebCrypto column live-checked; all 5 vectors PASS
- [x] `npm run assurance` - regenerated `docs/assurance-matrix.md` reflects the new spec sections, vector sources, test IDs, and lifecycle limitation on `aes` and `aes-gcm` rows
- [x] `npm run lint` - clean
- [x] `npm run build` - clean (main bundle 250 kB, gzipped 78 kB)
- [x] `python -c 'ast.parse(...)'` - `scripts/aes-interop/verify_python.py` parses cleanly
- [x] `bash -n scripts/aes-interop/verify_openssl.sh` - openssl helper parses cleanly
- [x] Review correction: matrix and README no longer claim OpenSSL CLI validates AES-GCM tags; OpenSSL CLI is limited to AES-ECB parity, with full GCM parity covered by WebCrypto and Python `cryptography`. The OpenSSL helper uses Node for JSON parsing so Windows Git Bash does not trip over Python Store aliases.
- [x] Spot-checked published `docs/aes-interop-matrix.md` and `docs/aes-lifecycle.md` for accuracy and tone

### Review
- Trust-signal gap closed with three artifacts: the deterministic interop suite (CI), the published matrix (recruiter-readable), and the lifecycle doc (NIST/OWASP-anchored).
- WebCrypto column is the only one re-validated live every run; Python/OpenSSL columns are reproducible on demand. That's the right trade for a static repo: the trust signal is "anyone can re-run the helpers and get the same answer," not "CI runs OpenSSL inside Vercel."
- Educational framing is reinforced in both new docs and in the updated assurance limitations. No production-safety claim added.
- TC4 (with AAD) was new to the suite; landing it lifts AES-GCM coverage from 2 of NIST's 4 published test cases to 3. TC1 was intentionally skipped (key/IV all-zero with empty plaintext is a degenerate authenticator-only check that TC2 already covers more sharply).

### Avoid (explicit)
- Do not lift the educational AES code into production guidance. Every new doc reinforces "use Web Crypto / libsodium / Tink."
- Do not chase WebCrypto parity at the cost of the toolkit's pedagogical clarity. Parity is a trust signal, not a refactor mandate.
